...
import io.github.mmm.test.ObjectHelper;
import io.github.mmm.test.TestValues;
...
public class CaseSyntaxTest extends Assertions {
  ...
  public void testOfCustomized() {
    ...
    assertThat(CaseSyntax.of(separator, ORIGINAL_CASE, UPPER_CASE, ORIGINAL_CASE).toString())
        .isEqualTo("$$Custom~Case");
  }
  ...
  private void verifyOfExampleStandardized(String example, CaseSyntax constant) {
    ...
    if (constant.getOtherCase() != CaseConversion.ORIGINAL_CASE) {
      assertThat(constant.convert("VariableName"))
          .isEqualTo(example.replace("" + CaseConversion.EXAMPLE_CHAR_FOR_ORIGINAL_CASE, ""));
    }
  }
  ...
  public void testOfExampleMinimal() {
    ...
    assertThat(CaseSyntax.ofExample("$$A", true))
        .isEqualTo(CaseSyntax.of(null, ORIGINAL_CASE, UPPER_CASE, ORIGINAL_CASE));
    assertThat(CaseSyntax.ofExample("$$a", true))
        .isEqualTo(CaseSyntax.of(null, ORIGINAL_CASE, LOWER_CASE, ORIGINAL_CASE));
    assertThat(CaseSyntax.ofExample("$A$", true))
        .isEqualTo(CaseSyntax.of(null, ORIGINAL_CASE, ORIGINAL_CASE, UPPER_CASE));
    assertThat(CaseSyntax.ofExample("$a$", true))
        .isEqualTo(CaseSyntax.of(null, ORIGINAL_CASE, ORIGINAL_CASE, LOWER_CASE));
    assertThat(CaseSyntax.ofExample("A$$", true)).isSameAs(CaseSyntax.CAPITALIZED);
    assertThat(CaseSyntax.ofExample("a$$", true))
        .isEqualTo(CaseSyntax.of(null, LOWER_CASE, ORIGINAL_CASE, ORIGINAL_CASE));
  }
  ...
  public void testCombined() {
    ...
    // edge cases
    assertThat(CaseSyntax.ofExample("_package", false).convert("mY.PacKagE_Name.With.Some_seGmentS"))
        .isEqualTo("my.package_name.with.some_segments");
  ...
  public void testOfExampleInvalidValues() {
    ...
    verifyExampleInvalid("InConSISTENTCase",
        ": the two characters 'SI' both have case UPPER_CASE but other case was detected as LOWER_CASE!");
    verifyExampleInvalid("INCONsistentcASE",
        ": the two characters 'si' both have case LOWER_CASE but other case was detected as UPPER_CASE!");
